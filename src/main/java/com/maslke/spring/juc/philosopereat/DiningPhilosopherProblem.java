...
public class DiningPhilosopherProblem {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < count; i++) {
            if (i != count - 1) {
                philosophers.add(new DeadlockingPhilosopher(i, chopsticks.get(i), chopsticks.get(i + 1)));
            }
            else {
                philosophers.add(new DeadlockingPhilosopher(i, chopsticks.get(i), chopsticks.get(0)));
            }
        }
